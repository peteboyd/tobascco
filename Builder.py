...
class Build(object):
    ...
    def sbu_orient(self, v, cell):
        ...
        if sbu.degree == 2 and not sbu.linear:
            ...
            n_edges = np.cross(arcs[0], arcs[1])
            n_edges = n_edges / np.linalg.norm(n_edges)
            n_sbu = np.cross(sbu_vects[0], sbu_vects[1])
            n_sbu = n_sbu / np.linalg.norm(n_sbu)
            angle = calc_angle(n_edges, n_sbu)
            axis = calc_axis(n_edges, n_sbu)
            R = rotation_matrix(axis, angle)
            mean, std = self.report_errors(sbu_vects, arcs, rot_mat=R)

            n_edges2 = np.cross(arcs[1], arcs[0])
            n_edges2 = n_edges2 / np.linalg.norm(n_edges2)

            angle2 = calc_angle(n_edges2, n_sbu)
            axis2 = calc_axis(n_edges2, n_sbu)
            R2 = rotation_matrix(axis2, angle2)

            mean2, std2 = self.report_errors(sbu_vects, [arcs[1], arcs[0]], rot_mat=R2)
