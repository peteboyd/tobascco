...
class Net(object):
    ...
    def init_params(self, init_guess):
        ...
        max_mt = self.metric_tensor[np.diag_indices_from(self.metric_tensor)].max()
        for (i,j) in zip(*np.triu_indices_from(self.metric_tensor)):
            val = self.metric_tensor[i,j]
            #val = np.identity(3)[i,j]
            if i == j:
                params.add("m_%i_%i"%(i,j), value=val, vary=False, min=0.001, max=self.metric_tensor[i][j])#max_mt) # NOT SURE WHAT THE MIN/MAX should be here!
            else:
                ...
                params.add("m_%i_%i"%(i,j), value=val, vary=False, min=-0.5, max=0.5)
        ...
        for (i,j), val in np.ndenumerate(init_guess):
            params.add("co_%i_%i"%(i+pp,j), value=val, vary=False, min=-0.2, max=0.2)
        return params
    ...
    def debug_embedding(self):
        ...
        for (i,j) in zip(*np.triu_indices(self.ndim, 1)):
            x[xinc] = self.metric_tensor[i,j]  / np.sqrt(self.metric_tensor[i,i]) \
                        /np.sqrt(self.metric_tensor[j,j])
            # set max and min angles to 120, 60 respectively.
            ub[xinc] = 0.5#max_cell**2
            lb[xinc] = -0.5#(max_cell**2)
            xinc += 1
