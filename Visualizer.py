...
class GraphPlot(object):
    ...
    def plot_2d_cell(self, origin=np.zeros(2), colour="b"):
        xyz_a = (self.cell[0] + origin) / 2.0
        xyz_b = (self.cell[1] + origin) / 2.0
        self.fig.text(xyz_a[0], xyz_a[1], "a", fontsize=self.fontsize)
        self.fig.text(xyz_b[0], xyz_b[1], "b", fontsize=self.fontsize)
        all_points = [
            np.sum(a, axis=0) + origin for a in list(self.powerset(self.cell)) if a
        ]
    ...
    def plot_3d_cell(self, origin=np.zeros(3), colour="b"):
        ...
        self.ax.text(
            xyz_b[0], xyz_b[1], xyz_b[2], "b", fontsize=self.fontsize, color=colour
        )
        self.ax.text(
            xyz_c[0] + 0.02,
            xyz_c[1],
    ...
    def view_placement(self, init=(0.0, 0.0, 0.0), edge_labels=True, sbu_only=[]):
        init = np.array(init)
        ...
        for key, value in unit_cell_vertices.items():
            if (sbu_only and key in sbu_only) or (not sbu_only):
                ...
                self.add_point(
                    p=np.array(value), label=None, colour="#FF6600"
                )  # Blaze Orange
