...
class CIF:
    ...
    def _readfile(self, filename):
        ...
        for line in filelines:
            ...
            if blockread and (
                line.startswith("loop_") or line.startswith("_") or not line
            ):
                blockread = False
    ...
    def insert_block_order(self, name, index=None):
        ...
        if index is None and name in self.block_order:
            ...
            index is not None
            and name in self.block_order
            and index < len(self.block_order)
        ):
            ...
            self.block_order.pop(old)
            index = len(self.block_order)
        self.block_order = self.block_order[:index] + [name] + self.block_order[index:]
    ...
    def __str__(self):
        ...
        for block in self.block_order:
            ...
            if block in self.non_loops:
                vals = zip(
                    [CIF.label(i) for i in heads], [self._data[i] for i in heads]
                )
            else:
